...
contract SrDegenDrink is ERC1155, Ownable, ERC1155Supply {
    ...
    function mint(
        ...
        require(
            _token.transferFrom(account, address(this), price),
            "SrDegenDrink: transfer failed"
        );
        _creators[id].creator = account;
        _tokenCIDs[id] = CID;
        _internalBalance += price;
    ...
    function mint(
        ...
        balances[_creators[id].creator] += price;
        _creators[id].value += price;
        _mint(account, id, amount, "");
    }

    function withdraw(uint256 id) external {
        require(
            _creators[id].creator == msg.sender,
            "SrDegenDrink: only creator can withdraw"
        );
        uint256 amount = _creators[id].value;
    ...
    function getTokenUri(uint256 tokenId) public view returns (string memory) {
        require(
            bytes(_tokenCIDs[tokenId]).length > 0,
            "MyERC1155Token: CID not set for token ID"
        );
